...
namespace ViewAPI.Controllers
{
    ...
    public class NhanVienController : BaseController<NhanVien, NhanVienDTO, string>
    {
        ...
        public async Task<ActionResult<NhanVienDTO>> Create([FromBody] NhanVienDTO dto)
        {
            ...
            // Lấy ChucVuId theo TenChucVu (không phân biệt hoa/thường), nếu không tìm thấy thì không cho lưu để tránh Guid.Empty gây lỗi FK
            var chucVuId = await _context.chucVus
                .Where(x => x.Ten.ToLower() == dto.TenChucVu!.ToLower())
                .Select(x => x.Id)
                .FirstOrDefaultAsync();

            if (chucVuId == Guid.Empty)
            {
                return BadRequest("Không tìm thấy chức vụ phù hợp. Vui lòng chọn lại chức vụ.");
            }
            // 2️⃣ Tạo NhanVien
        ...
        public async Task<IActionResult> UpdateCustomer([FromBody] NhanVienDTO dto)
        {
            ...
            // Lấy lại ChucVuId theo TenChucVu (không phân biệt hoa/thường), nếu không có thì không cho cập nhật
            var chucVuId = await _context.chucVus
                .Where(x => x.Ten.ToLower() == dto.TenChucVu!.ToLower())
                .Select(x => x.Id)
                .FirstOrDefaultAsync();
